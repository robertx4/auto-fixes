...
async def generate_dl_command(
  ...
  if (any(audio_format in line and "[es" not in line for line in format_info) or
...
async def Youtube_CLI(message: Message, msg):
  url = message.text
  ...
  for attempt in range(retries + 1):
    try:
      ...
      if all(
          "[info] Available formats for " not in line for line in format_info):
